...
# NOTE: It seems to work, but now we only need to 
        ...
        if line == '$ ls':
            ...
            if i < 5000000: #16 if using input_test.txt
                ...
                if splitValues[0] == '$': # we pass ls but keeping for other items
                    if splitValues[1] == 'cd':
                        if sum(currentValues) > 100000: # we only want to record directories over 100,000
                            parentBuckets = []
                            ...
                            if trueParent in df['TopParent'].values:
                                for i in range(0,len(df)):
                                    if df['TopParent'][i] == trueParent:
                                        df['TotalValue'][i] = df['TotalValue'][i] + int(sum(currentValues))
                            else:
                                df = df.append({'TopParent':trueParent, 'TotalValue':int(sum(currentValues))},ignore_index=True)
                        currentValues = [] #clear it out
